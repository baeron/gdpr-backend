...
export class FormAnalyzer {
  ...
  async analyzeForms(page: Page): Promise<FormsAnalysisResult> {
    ...
    for (const pageUrl of additionalPages.slice(0, 5)) {
      ...
      if (pageUrl === baseUrl || pageUrl === origin || pageUrl === origin + '/')
        continue;
    ...
    result.dataCollectionForms = result.forms.filter(
      (f) => f.hasEmailField || f.hasNameField || f.hasPhoneField,
    ).length;
    result.formsWithConsent = result.forms.filter(
      (f) => f.hasConsentCheckbox && (f.hasEmailField || f.hasNameField),
    ).length;
    result.formsWithoutConsent = result.forms.filter(
      (f) => (f.hasEmailField || f.hasNameField) && !f.hasConsentCheckbox,
    ).length;
    result.formsWithPreCheckedMarketing = result.forms.filter(
      (f) => f.hasPreCheckedMarketing,
    ).length;
    result.formsWithPrivacyLink = result.forms.filter(
      (f) => f.hasPrivacyPolicyLink && (f.hasEmailField || f.hasNameField),
    ).length;
      ...
      const formData = await form.evaluate((el: HTMLFormElement) => {
        ...
        checkboxes.forEach((checkbox: HTMLInputElement) => {
          ...
          const isConsent = [
            ...
            'terms',
            'gdpr',
          ].some((kw) => checkboxText.includes(kw));
  ...
  private determineFormType(formData: any): FormInfo['type'] {
    ...
    if (
      formData.hasEmailField &&
      !formData.hasNameField &&
      !formData.hasMessageField
    ) {
      return 'newsletter';
    }
  ...
  private async findFormPages(page: Page): Promise<string[]> {
    ...
    try {
      ...
      for (const link of links) {
        ...
        const isFormPage = formPageKeywords.some(
